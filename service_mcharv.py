...
def user_input_base_url():
    ...
    input_url = input('Base URL (Metacritic only), no slash at the end please:\n')
    # pass the URL out of the function so it can be used elsewhere
    return input_url

def create_working_url(raw_url):
    passing_url = F"{raw_url}/user-reviews?sort-by=date&num_items=100&page=" # note that a page number is not supplied, but will be generated by the program later
    print(F'We will start working on the reviews on the first page, which is located at {passing_url}0.') # Metacritic review pages start numbering at 0, but the first page is still displayed as 1 - this is why I have users provide the raw URL
